...
public class TravellerDao implements Messages {
	...
	public boolean addTraveller (Person person, Traveller traveller) throws Fly5Exception{
		...
		String query= "insert into person(first_name,last_name,street,city,state,zip,date_of_birth,email_address,password) values (?,?,?,?,?,?,?,?,?) ";
...
System.out.println("Same id"+checkAvailability);
	...
	public int authenticateTraveller(String email,String password) throws Fly5Exception{
		...
		try{
			...
			con = ConnectionFactory.getInstance().getConnection();
			con.setAutoCommit(false);
			ps=con.prepareStatement("Select * from person where email_address = ? && password = ?");
	...
	public boolean updateTraveller (Person person, Traveller traveller) throws Fly5Exception{
		...
		try{
			...
			ps1=con.prepareStatement("select * from person where email_address = ?  ");
			...
			ps=con.prepareStatement("update person set first_name = ?,last_name = ?,street = ?,city = ?,state = ?,zip = ?,date_of_birth = ?, password = ? where email_address = ?");
			ps.setString(1,	person.getFirstName());
